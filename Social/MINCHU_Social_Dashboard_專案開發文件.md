...
| 頁籤名稱 | 功能說明 |
|----------|---------|
| 總覽 | 年度 KPI 卡片（粉絲總數、本月觸及、新增追蹤、互動）、YTD 年度累計卡片（觸及/追蹤 vs 年度目標進度條，含目標來源標籤）、歷年粉絲成長趨勢（圖表/表格切換，表格含 FB/IG/總 成長數與成長率） |
...
| 貼文分析 | Top 5 貼文卡片（可切換全部/FB/IG）、篩選器（平台/分類/排序，分組式 UI）、排序選項含「月份」可收合分組（顯示月匯總：篇數、總觸及、總互動、篇均觸及）、批次分類、分類成效比較圖。平台欄位以彩色標籤區分（FB 藍/IG 粉）。觸及衰退追蹤（W1/W2 觸及 + 累積觸及三欄，Phase 2 自動撈取）。 |
...
每篇貼文發布後，系統於第 7 天和第 14 天各撈取一次觸及數據，追蹤觸及衰退曲線：

| 欄位 | 意義 | 撈取時機 | API 方式 |
|------|------|---------|---------|
| W1 觸及 | 發布後第 1-7 天的累積觸及 | 發布 +7 天 | `GET /{post-id}/insights` |
| W2 觸及 | 第 8-14 天新增的觸及 | 發布 +14 天 | 累積值 - W1 |
| 累積觸及 | 截至最後一次撈取的總觸及 | = W1 + W2 | 同上 |

- 每篇貼文僅需 **2 次 API call**（非每日撈取），大幅節省 Rate Limit
- Edge Function 每日檢查哪些貼文剛好滿 7 天或 14 天，僅對符合條件的貼文撈取
- 可分析「長尾型」vs「爆發型」貼文的內容特徵
- Demo 版已預留 W1/W2 欄位結構，值顯示「待接入」
